...
export default function ContactForm() {
  ...
  useEffect(() => {
    // Primary source: api.country.is (Fast & CORS friendly)
    fetch('https://api.country.is')
      ...
      .catch((err) => {
        console.warn('Primary detection failed, trying fallback...', err);
        ...
        fetch('/api/detect-country')
          ...
          .catch(() => console.error('All country detection methods failed'));
      });
  }, []);
  ...
  return (
    <div id="contact-form" className="self-stretch">
      ...
      {/* Мобільна версія */}
      <div
        className="xl:hidden self-stretch bg-[url('/fantastic-seascape-with-ripples-depositphotos-bgremover%201.png')] bg-cover py-[74px] md:py-20 lg:py-24 px-5 md:px-8 lg:px-12"
      ...
      >
        <div className="flex flex-col self-stretch md:max-w-[600px] lg:max-w-[700px] md:mx-auto py-7 md:py-10 lg:py-12 px-5 md:px-8 lg:px-10 rounded-[15px] md:rounded-[20px] bg-[#011b2ae6]">
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            <div className="flex w-full gap-3 md:gap-4 lg:gap-5 mb-[30px] md:mb-8 lg:mb-10 h-[50px] md:h-[55px] lg:h-[60px]">
              <PhoneInput
                key={countryCode}
                country={countryCode}
                value={phone}
            ...
            <div className="flex flex-col items-start self-stretch mb-[18px] md:mb-5 lg:mb-6 gap-[7px] md:gap-2.5 lg:gap-3">
              ...
              <div className="flex items-center self-stretch gap-0 rounded-md md:rounded-lg bg-[#FFFFFF1A]">
                <button
                  ...
                  className={`flex flex-1 items-center justify-center gap-1.5 py-[11px] border-0 rounded-md md:rounded-lg relative overflow-hidden transition-all ${contactMethod === "whatsapp" ? "bg-[#25D366]" : ""}`}
                >
                  ...
                  <span className="text-white text-sm md:text-base lg:text-lg font-extrabold relative z-10">
                    WhatsApp
                  </span>
                </button>
                <button
                  ...
                  className={`flex flex-1 items-center justify-center gap-1.5 py-[11px] border-0 rounded-md md:rounded-lg relative overflow-hidden transition-all ${contactMethod === "telegram" ? "bg-[#35ACE1]" : ""}`}
                >
                  <svg className="w-5 h-5 shrink-0 text-white" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div
        className="hidden md:flex flex-col items-center self-stretch bg-[url('/fantastic-seascape-with-ripples-depositphotos-bgremover%201.png')] bg-cover py-12 lg:py-16 xl:py-24 2xl:py-19 px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-13 relative"
          ...
          <div className="flex-1 flex flex-col justify-between p-8 md:p-8 lg:p-12 xl:p-20 2xl:p-16 xl:pr-10 2xl:pr-8">
            <div className="mb-8 md:mb-10 xl:mb-0">
              <h2 className="text-white text-[28px] md:text-[32px] lg:text-[40px] xl:text-[48px] 2xl:text-[38px] font-bold leading-[110%] uppercase mb-4 md:mb-6 2xl:mb-5 max-w-[550px] 2xl:max-w-[440px]">
                Станьте владельцем{" "}
                <span className="text-[#82D3FF]">недвижимости у моря</span>
              </h2>
            ...
            <div className="relative">
              ...
              <div className="bg-[#ffffff08] backdrop-blur-[2px] border border-[#ffffff20] rounded-[20px] 2xl:rounded-[16px] p-6 lg:p-8 2xl:p-6 flex flex-col gap-4 lg:gap-6 2xl:gap-5 relative z-10 overflow-hidden">
                ...
                <div className="flex flex-row items-stretch justify-center gap-2 md:gap-4 lg:gap-6 2xl:gap-10 relative z-10 w-full">
                  <IconPoint text="актуальные" boldText="цены" />
                  ...
                  />
                  <div className="w-[1px] bg-[linear-gradient(180deg,rgba(255,255,255,0)_0%,#FFFFFF_50%,rgba(255,255,255,0)_100%)] opacity-40"></div>
                  <IconPoint text="расчёт" boldText="доходности" />
                </div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="bg-[#ffffff0a] backdrop-blur-[2px] border border-[#ffffff20] rounded-[24px] xl:rounded-[30px] 2xl:rounded-[24px] p-6 md:p-6 lg:p-8 xl:p-10 2xl:p-8 flex flex-col gap-6 xl:gap-8 2xl:gap-6 relative overflow-hidden">
                ...
                <div className="flex flex-col gap-4 2xl:gap-3 relative z-10 w-full mb-2 2xl:mb-1">
                  ...
                  <div className="flex items-stretch self-stretch rounded-xl 2xl:rounded-[10px] border-x-[1px] border-y-[1px] border-[#ffffff40] bg-[#FFFFFF0A] overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setContactMethod("whatsapp")}
                      className={`flex-1 flex items-center justify-center gap-3 2xl:gap-2 h-14 2xl:h-11 rounded-xl 2xl:rounded-[10px] transition-all cursor-pointer ${contactMethod === "whatsapp" ? "bg-[#25D366] text-white border-0" : "bg-transparent text-white/80 border-0"}`}
                    >
                      <svg
                        className="w-6 h-6 2xl:w-5 2xl:h-5"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                      >
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                      </svg>
                      <span className="font-bold text-[18px] 2xl:text-[14px]">
                        WhatsApp
                      </span>
                    </button>
                    <button
                      type="button"
                      onClick={() => setContactMethod("telegram")}
                      className={`flex-1 flex items-center justify-center gap-3 2xl:gap-2 h-14 2xl:h-11 rounded-xl 2xl:rounded-[10px] transition-all cursor-pointer ${contactMethod === "telegram" ? "bg-[#35ACE1] text-white border-0" : "bg-transparent text-white/80 border-0"}`}
                    >
                      <svg
                        className="w-6 h-6 2xl:w-5 2xl:h-5"
                        viewBox="0 0 24 24"
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full h-16 2xl:h-13 bg-[linear-gradient(90deg,#FFE643_0%,#FFEE7D_51.92%,#FFE643_100%)] rounded-[15px] 2xl:rounded-[12px] border-0 shadow-lg cursor-pointer hover:brightness-105 active:scale-[0.98] transition-all relative z-10 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span className="text-[#453E0C] text-[20px] 2xl:text-[16px] font-bold">
                    {isSubmitting ? "Отправка..." : "Получить подборку"}
                  </span>
                </button>
              </div>
...
function IconPoint({ text, boldText }: { text: string; boldText?: string }) {
  return (
